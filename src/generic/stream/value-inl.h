...
really_inline simdjson_result<value&>::simdjson_result(stream::value &value) noexcept
    : internal::simdjson_result_base<stream::value&>(value) {}
really_inline simdjson_result<value&>::simdjson_result(stream::value &value, error_code error) noexcept
    : internal::simdjson_result_base<stream::value&>(value, error) {}

really_inline array simdjson_result<value&>::begin() noexcept {
  return array::begin(first, error());
}
really_inline array simdjson_result<value&>::end() noexcept {
  return {};
}
really_inline simdjson_result<value&> simdjson_result<value&>::operator[](std::string_view key) && noexcept {
  if (error()) { return *this; }
  return std::forward<stream::value>(first)[key];
}

really_inline simdjson_result<array> simdjson_result<value&>::get_array() && noexcept { return error() ? simdjson_result<array>(first, error()) : std::forward<stream::value>(first).get_array(); }
really_inline simdjson_result<object> simdjson_result<value&>::get_object() && noexcept { return error() ? simdjson_result<object>(first, error()) : std::forward<stream::value>(first).get_object(); }
really_inline simdjson_result<uint64_t> simdjson_result<value&>::get_uint64() && noexcept { return error() ? error() : std::forward<stream::value>(first).get_uint64(); }
really_inline simdjson_result<int64_t> simdjson_result<value&>::get_int64() && noexcept { return error() ? error() : std::forward<stream::value>(first).get_int64(); }
really_inline simdjson_result<double> simdjson_result<value&>::get_double() && noexcept { return error() ? error() : std::forward<stream::value>(first).get_double(); }
really_inline simdjson_result<std::string_view> simdjson_result<value&>::get_string() && noexcept { return error() ? error() : std::forward<stream::value>(first).get_string(); }
really_inline simdjson_result<raw_json_string> simdjson_result<value&>::get_raw_json_string() && noexcept { return error() ? error() : std::forward<stream::value>(first).get_raw_json_string(); }
really_inline simdjson_result<bool> simdjson_result<value&>::get_bool() && noexcept { return error() ? error() : std::forward<stream::value>(first).get_bool(); }

#if SIMDJSON_EXCEPTIONS
really_inline simdjson_result<value&>::operator array() && noexcept(false) {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_array();
}
really_inline simdjson_result<value&>::operator object() && noexcept(false) {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_object();
}
really_inline simdjson_result<value&>::operator uint64_t() && noexcept(false)  {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_uint64();
}
really_inline simdjson_result<value&>::operator int64_t() && noexcept(false)  {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_int64();
}
really_inline simdjson_result<value&>::operator double() && noexcept(false)  {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_double();
}
really_inline simdjson_result<value&>::operator std::string_view() && noexcept(false)  {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_string();
}
really_inline simdjson_result<value&>::operator raw_json_string() && noexcept(false)  {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_raw_json_string();
}
really_inline simdjson_result<value&>::operator bool() && noexcept(false) {
  if (error()) { throw simdjson_error(error()); }
  return std::forward<simdjson_result<stream::value&>>(*this).get_bool();
}
#endif
