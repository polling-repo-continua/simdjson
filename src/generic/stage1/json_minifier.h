...
private:
  ...
  really_inline void next(const simd::simd8x64<uint8_t>& in, const json_block& block);
  really_inline error_code finish(uint8_t *dst_start, size_t &dst_len);
  json_scanner scanner{};
  uint8_t *dst;
};

really_inline void json_minifier::next(const simd::simd8x64<uint8_t>& in, const json_block& block) {
